// One text from the club number, recorded in the SMS inbox.
//
...
export async function threadFor(supabase, r) {
  ...
  if (!thread) {
    const ins = await supabase.from("sms_threads").insert({
      phone: r.to, player_id: r.player_id || null, team_name: r.team_name || null, contact_kind: r.kind || null, contact_name: r.name || null,
    }).select().single();
...
// r: { to (E.164), name?, kind?, player_id?, team_name? }
// meta: { broadcast_id?, sent_by_coach_id?, sent_by_label? }
...
export async function sendOneSms(supabase, r, text, meta = {}) {
  const { TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER } = process.env;
  if (!twilioReady()) throw new Error("Twilio not configured. Set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_FROM_NUMBER.");
  const thread = await threadFor(supabase, r);
  ...
  const msgInsert = await supabase.from("sms_messages").insert({
    thread_id: thread.id, direction: "outbound", body: text, status: "queued",
  ...
  await supabase.from("sms_threads").update({ last_message_at: now, last_message_preview: preview(text), last_message_direction: "outbound", updated_at: now }).eq("id", thread.id);
