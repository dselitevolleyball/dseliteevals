...
DSSC CRM (People): dssc_contacts(id, email, first_name, last_name, phone, city, zip, dob, sources text[] playbook|upperhand|dse|form, tags text[], notes, do_not_text, playbook_user_pk) = families/account holders; dssc_participants(id, contact_id, first_name, last_name, dob, gender, is_contact, dse_player_id, playbook_student_pk, position, position2, dse_team, dse_level rise|regional|national) = the players (kids) under a family; dssc_participation(participant_id, contact_id, program, category volleyball|basketball|reach|facility|other, event_date, source playbook|upperhand|dse) = every program/class/season someone took part in; dssc_orders(contact_id, buyer, total_cents, ordered_at, sale_source) = Upper Hand purchases (no line items). Age = from dssc_participants.dob. "Can we text them" = dssc_contacts.phone plus sms_consents where brand='dssc'.
